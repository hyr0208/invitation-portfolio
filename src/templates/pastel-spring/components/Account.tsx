    ...
    <div className="flex items-center justify-between py-3">
      <div>
        <p className="text-sm text-spring-ink">
          <span className="mr-2 text-spring-ink-faint">{account.label}</span>
          {account.holder}
        </p>
        <p className="mt-0.5 text-xs text-spring-ink-soft">
...
function AccountGroup({ side, accounts }: { side: string; accounts: AccountInfo[] }) {
  ...
  return (
    <div className="border-b border-spring-line py-4 last:border-b-0">
      <button
        type="button"
        onClick={() => setOpen((prev) => !prev)}
        className="flex w-full items-center justify-between text-sm font-medium text-spring-ink"
      >
        {side}
      ...
      >
        <div className="overflow-hidden">
          <div
            className={`divide-y divide-spring-line/70 transition-opacity duration-300 ${
...
export function Account({ data }: AccountProps) {
  ...
  return (
    <Section index="06" eyebrow="Gift" title="마음 전하실 곳">
      <p className="mb-6 text-center text-sm leading-relaxed text-spring-ink-soft">
