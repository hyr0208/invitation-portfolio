    ...
    <div className="flex items-center justify-between py-3">
      <div>
        <p className="text-sm text-rose-ink">
          <span className="mr-2 text-rose-ink-faint">{account.label}</span>
          {account.holder}
        </p>
        <p className="mt-0.5 text-xs text-rose-ink-soft">
...
function AccountGroup({ side, accounts }: { side: string; accounts: AccountInfo[] }) {
  ...
  return (
    <div className="border-b border-blossom-line py-4 last:border-b-0">
      <button
        type="button"
        onClick={() => setOpen((prev) => !prev)}
        className="flex w-full items-center justify-between text-sm font-medium text-rose-ink"
      >
        {side}
      ...
      >
        <div className="overflow-hidden">
          <div
            className={`divide-y divide-blossom-line/70 transition-opacity duration-300 ${
...
export function Account({ data }: AccountProps) {
  ...
  return (
    <Section index="06" eyebrow="Gift" title="마음 전하실 곳">
      <p className="mb-6 text-sm leading-relaxed text-rose-ink-soft">
